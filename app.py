...
if not os.path.exists("healthcare_data.csv"):
    ...
    df["Paid_Amount"] = df["Claim_Amount"] * np.random.uniform(0.4, 1.0, n)
    df["Revenue_Leakage"] = df["Claim_Amount"] - df["Paid_Amount"]
    df["Claim_Denied"] = np.where(
        df["Paid_Amount"] < df["Claim_Amount"]*0.6, 1, 0)
